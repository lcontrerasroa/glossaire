...
def main():
    ...
    for i, row in filtered.iterrows():
        with st.expander(f"{row['term']} ({row['langue']})"):
            ...
            # Suggestion depuis le backend web
            if st.button("Définition enrichie depuis le web", key=gen_key):
            ...
            if web_def_key in st.session_state:
                st.markdown("**Suggestion enrichie :**")
                ...
                if st.button("Utiliser cette suggestion", key=use_web_def_key):
                    ...
                    new_def = st.session_state[web_def_key]
